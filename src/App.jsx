...
{/* LEADS DEL PORTAL */}
        {tab === "leads_portal" && (
          <div>
            ...
            {leads.map((lead, i) => {
              const sel = leadSel?.id === lead.id && !leadSel?.editando;
              const editando = leadSel?.id === lead.id && leadSel?.editando;
              console.log("LEAD", lead.id, "sel:", sel, "editando:", editando, "leadSel:", leadSel);
              ...
              return (
               <div key={lead.id || i}onClick={e => { if (e.target.tagName === "BUTTON" || e.target.closest("button")) return; if (!editando) setLeadSel(leadSel?.id === lead.id && !leadSel?.editando ? null : lead); }}
